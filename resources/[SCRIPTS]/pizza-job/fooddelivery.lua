...
local foodLocations = {
    ...
    {
        coords = vector4(81.2722, 274.1758, 110.2102, 161.3865),
        spawnCoords = vector4(80.0236, 265.2462, 108.7634, 156.2941),
        ped = nil,
        deliveryPoints = {
            vector4(-1309.0474, -930.8871, 13.3582, 110.3461),
            vector4(-1319.8501, -934.9029, 16.3586, 22.1771),
...
function GetRandomPedToDeliver(coords)
    ...
    pedModelDelivery = math.random(1, #availablePeds)
    RequestModel(pedModelDelivery)
    while not HasModelLoaded(pedModelDelivery) do
        Citizen.Wait(1)
    end
    local pedSpawn = CreatePed(26, GetHashKey(pedModelDelivery), coords.x, coords.y, coords.z - 1, coords.w, false, true)
    SetEntityInvincible(pedSpawn, true)
    location.ped = pedSpawn
end
        ...
        if IsPedInAnyVehicle(playerPed) and currentDepot then
            ...
            if deliveryMissionState == 6 and distance < 50 then
                ...
                if distance < 5 then
                    ...
                    local deliveryData = json.decode(GetExternalKvpString("save-load", "FOODDELIVERY_DATA"))
